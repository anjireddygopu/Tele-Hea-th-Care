	...
	$('#states').on("change", function (){
	  ...
	  $.ajax({
	     ...
	     { 
	      var result=JSON.parse(json);
	      $("#districts").empty();
	      $("#districts").append("<option value=>--District-Name--</option>");
	      $.each(result, function(index, item)
	     {
	...
	$('#states1').on("change", function (){ 
	  ...
	  $.ajax({
	     ...
	     { 
	      var result=JSON.parse(json);
	      $("#districts1").empty();
	      $("#districts1").append("<option value=>--District-Name--</option>");
	      $.each(result, function(index, item)
	     {
	     $("#districts1").append("<option value="+item.district_id+">"+item.district_name+ "</option>");
	     }); 
	            ...
	            oTable = $('#example23').DataTable({
		            ...
		            { "data": "town_name"},
		            {
		                ...
		                render: function (data, type, row) {
		                    if (data == true) {
		                        return "<input type='checkbox' id='" + row.town_id + "_active' checked='checked'/>"
		                    } else {
		                        return "<input type='checkbox' id='" + row.town_id + "_active'/>"
		                    };
		                }
		            },
		            {
		            	   ...
		            	   render: function (data, type, row) {
		            	       ...
		            	       return "<button id='" + row.town_id + "'_edit' onclick='edit(this.id)'><i class='fa fa-pencil-square-o btn-primary btn-sm' data-toggle='tooltip' title='Edit'></i></button>"
...
function save(){
	...
	+dNow.getFullYear()+""
	+dNow.getHours();
	var postData=JSON.stringify({ "town_name":town_name,"country_id":country_id,"state_id":state_id,"district_id":district_id,"created_by":"anji","created_on":localdate,"active":active});
	$.ajax({
	url:"https://"+ipadd+":3000/api/v1/Towns",
	type:"POST",
	data: postData ,
	...
	$(function(){
		if(!(id==null)){
			 $.ajax({
			          type:"GET",
			          headers: {'x-access-token':token},
			          url:"https://"+ipadd+":3000/api/v1/Towns/"+id,
			          data:""})
			          .done(function(json){
...
function update(){
	...
	+dNow.getHours();
	var user="siva"
	var postData=JSON.stringify({"town_id":id,"town_name":town_name,"district_id":district_id,"country_id":country_id,"state_id":state_id,"updated_by":user,"updated_on":localdate,"active":active })
	$.ajax({
	url:"https://"+ipadd+":3000/api/v1/Towns/",
	type:"PUT",
	data: postData ,
