...
var ipadd=sessionStorage.getItem("ip-add");
		...
		function ShowLists() {
			...
			$.ajax({
						"type" : "GET",
						"url" : "https://"+ipadd+":3000/api/v1/satellite",
						"data" : "",
						"dataSrc" : "",
						headers : {
														...
														},
														{
															...
															render : function(
																...
																return "<button id='"
																		...
																		+ "_edit' onclick='editsatellites("
																		+ row.facility_center_id
																		+ ")'><i class='fa fa-pencil-square-o btn-primary btn-sm' data-toggle='tooltip' title='Edit'></i></button>"
		...
		function editsatellites(id) {
			$("#list").hide();
			$("#add").hide();
			$("#badd").hide();
			$("#edit").show();
			
			sessionStorage.setItem("satellite_id", id);
			var id = sessionStorage.getItem("satellite_id");
			if (!(id == null)) {
				$.ajax({
							type : "GET",
							url : "https://"+ipadd+":3000/api/v1/satellite/"
						...
						.done(function(json) {
								...
								$.each(data, function(i, obj) {
									$("#id1").val(obj.facility_center_id);
									$("#name1").val(obj.facility_center_name);
									$("#check11").val(obj.active);
									if (obj.active == 1) {
									...
									$("#hmno1").val(obj.address_line1);
									$("#sname1").val(obj.address_line2);
									$("#country1").find(
									...
									$("#city1").find(
											...
											.prop("selected", true);
									$("#pcode1").val(obj.zip);
									$("#longi1").val(obj.longitude);
									$("#lati1").val(obj.latitude);
									$("#cname1").val(obj.contact_person_name);
									$("#email1").val(obj.email_id);
									$("#mobile1").val(obj.contact_no1);
									$("#phno1").val(obj.contact_no2);
									$("#fno1").val(obj.fax);
		...
		function editsatellite() {
			var fid = sessionStorage.getItem("satellite_id");
			var name = $("#name1").val();
			...
			$.ajax({
						type : "PUT",
						url : "https://"+ipadd+":3000/api/v1/satellite/update",
						headers : {
							"x-access-token":token
						},
						data :JSON.stringify(data),
